...
// src/controllers/reports.controller.ts
// ARQUIVO: kidpark-api/src/controllers/reports.controller.ts
// Corrigido: tipagem explícita em todos os callbacks (strict/noImplicitAny)

import { Request, Response } from 'express'
import { prisma } from '../lib/prisma'
import type { ChildSession, Toy, User } from '@prisma/client'
...
// Tipo da sessão com os relacionamentos incluídos
type SessionWithRelations = ChildSession & {
  toy:  Toy
...
export async function daily(_req: Request, res: Response) {
  ...
  const sessions: SessionWithRelations[] = await prisma.childSession.findMany({
    where:   { createdAt: { gte: start, lte: end } },
    include: { toy: true, user: { select: { id: true, name: true } } },
  })
  ...
  const porStatus = sessions.reduce((acc: Record<string, number>, s: SessionWithRelations) => {
    acc[s.status] = (acc[s.status] ?? 0) + 1
    return acc
  }, {} as Record<string, number>)
  ...
  res.json({
    ...
    data: {
      totalTickets:      sessions.length,
      totalArrecadado,
      pendentePagamento: sessions.filter((s: SessionWithRelations) => s.paymentStatus === 'PENDING').length,
      brinquedos:        Object.values(toyMap).sort((a, b) => b.total - a.total),
      atendentes:        Object.values(attMap).sort((a, b) => b.total - a.total),
      porStatus,
      ultimaAtualizacao: new Date().toISOString(),
